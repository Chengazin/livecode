<template>
  <section class="card admin-section">
    <div class="sidebar-head">
      <div>
        <h2>{{ t("admin.terminalSessionsTitle") }}</h2>
        <p class="muted-text">{{ t("admin.terminalSessionsLead") }}</p>
      </div>
      <div class="admin-head-actions">
        <button class="btn btn-sm btn-secondary" type="button" @click="toggleForm">
          {{ formOpen ? t("common.cancel") : t("admin.terminalCreateAction") }}
        </button>
    ...
    <div v-if="isOpen" class="admin-section-body">
      <form class="admin-filter-row" @submit.prevent="applyFilters">
        <label class="field">
          ...
          <input v-model.trim="filters.search" type="text" :placeholder="t('admin.terminalSearchPlaceholder')" />
        </label>
        <label class="field">
        ...
        <label class="field">
          ...
          <select v-model="filters.status">
            ...
            <option value="open">{{ t("admin.terminalStatusOpen") }}</option>
            <option value="closed">{{ t("admin.terminalStatusClosed") }}</option>
          </select>
        </label>
        <label class="field">
          <span>{{ t("admin.terminalSharedLabel") }}</span>
          <select v-model="filters.shared">
            <option value="">{{ t("admin.filterAll") }}</option>
            <option value="true">{{ t("admin.terminalSharedYes") }}</option>
            <option value="false">{{ t("admin.terminalSharedNo") }}</option>
          </select>
        </label>
      ...
      <form v-if="formOpen" class="admin-editor-grid" @submit.prevent="submitForm">
        ...
        <label class="field">
          <span>{{ t("admin.terminalNameLabel") }}</span>
          <input v-model.trim="form.name" type="text" maxlength="120" required />
        </label>
        <label class="field">
          <span>{{ t("admin.terminalShellLabel") }}</span>
          <input v-model.trim="form.shell" type="text" maxlength="120" />
        </label>
        <label class="field">
          <span>{{ t("admin.terminalCwdLabel") }}</span>
          <input v-model.trim="form.cwd" type="text" maxlength="2048" />
        </label>
        <label class="field">
          <span>{{ t("admin.terminalSharedLabel") }}</span>
        ...
        <div class="admin-editor-actions">
          <button class="btn" type="submit" :disabled="saving">
            {{ saving ? t("common.saving") : (form.mode === "create" ? t("admin.terminalCreateSubmit") : t("admin.terminalUpdateSubmit")) }}
          </button>
      ...
      <div class="admin-table-wrap">
        <table class="admin-table">
          <thead>
            <tr>
              <th>{{ t("admin.tableTerminalSessionId") }}</th>
              <th>{{ t("admin.tableProjectId") }}</th>
              <th>{{ t("admin.tableUser") }}</th>
              <th>{{ t("admin.terminalNameLabel") }}</th>
              <th>{{ t("admin.terminalCwdLabel") }}</th>
              <th>{{ t("admin.terminalSharedLabel") }}</th>
              <th>{{ t("admin.userStatusLabel") }}</th>
              <th>{{ t("admin.tableUpdated") }}</th>
              <th>{{ t("admin.actionsLabel") }}</th>
            </tr>
          </thead>
          <tbody>
            <tr v-if="loading">
              <td colspan="9">{{ t("admin.loadingTerminalSessions") }}</td>
            </tr>
            <tr v-else-if="items.length === 0">
              <td colspan="9">{{ t("admin.noTerminalSessions") }}</td>
            </tr>
            <tr v-for="item in items" :key="item.terminal_session_id">
              <td>{{ item.terminal_session_id }}</td>
              <td>
              ...
              <td>
                ...
                <br />
                <small class="muted-text">{{ item.shell || "-" }}</small>
              </td>
              <td><span class="admin-path">{{ item.cwd || "-" }}</span></td>
              <td>{{ item.shared ? t("admin.terminalSharedYes") : t("admin.terminalSharedNo") }}</td>
              <td>{{ item.status || "-" }}</td>
              ...
              <td>
                <div class="admin-row-actions">
                  ...
                  <button class="btn btn-sm btn-danger" type="button" :disabled="saving || deletingId === Number(item.terminal_session_id || 0)" @click="deleteItem(item)">
                    {{ deletingId === Number(item.terminal_session_id || 0) ? t("common.saving") : t("admin.deleteAction") }}
                  </button>
                </div>
              </td>
            </tr>
...
function startEdit(item) {
  ...
  formOpen.value = true;
  form.mode = "edit";
  form.terminalSessionId = Number(item?.terminal_session_id || 0);
  form.projectId = Number(item?.project_id || 1);
  form.userId = Number(item?.user_id || 1);
  form.name = String(item?.name || "Terminal");
  form.shell = String(item?.shell || "");
  form.cwd = String(item?.cwd || "/");
  form.shared = Boolean(item?.shared);
  form.status = String(item?.status || "open");
}
...
async function submitForm() {
  ...
  try {
    if (form.mode === "create") {
      ...
      notice.value = t("admin.terminalCreated");
      closeForm();
      await loadItems(1);
      return;
    }
    ...
    await request({
      method: "PATCH",
      path: `/admin-terminal-sessions/${form.terminalSessionId}`,
      auth: true,
      body: payload,
    });

    notice.value = t("admin.terminalUpdated");
    closeForm();
...
async function deleteItem(item) {
  const terminalSessionId = Number(item?.terminal_session_id || 0);
  if (!terminalSessionId) {
    return;
  }

  const firstConfirm = window.confirm(t("admin.terminalDeleteConfirmStepOne", { id: terminalSessionId }));
  if (!firstConfirm) {
    return;
  }

  const secondConfirm = window.confirm(t("admin.terminalDeleteConfirmStepTwo"));
  if (!secondConfirm) {
    return;
  }

  deletingId.value = terminalSessionId;
  error.value = "";
  notice.value = "";
  ...
  try {
    await request({
      method: "DELETE",
      path: `/admin-terminal-sessions/${terminalSessionId}`,
      auth: true,
    });

    notice.value = t("admin.terminalDeleted");
    await loadItems(page.value);
  } catch (requestError) {
    error.value = readError(requestError);
  } finally {

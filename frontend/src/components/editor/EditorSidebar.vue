  ...
  <div class="editor-sidebar-shell">
    <nav class="editor-activity-bar" aria-label="Editor sidebar sections">
      ...
      <button
        ...
        class="editor-activity-btn"
        :class="{ 'is-active': activePanel === 'collaborators' }"
        type="button"
        :title="t('editor.collaborators')"
        :aria-label="t('editor.collaborators')"
        @click="setActivePanel('collaborators')"
      >
        <svg viewBox="0 0 24 24" aria-hidden="true">
          <path d="M16.5 11a3 3 0 1 0 0-6 3 3 0 0 0 0 6Z" />
          <path d="M7.5 12.5a2.5 2.5 0 1 0 0-5 2.5 2.5 0 0 0 0 5Z" />
          <path d="M4 19.5a4 4 0 0 1 7 0" />
          <path d="M12 19.5a4.5 4.5 0 0 1 8 0" />
        </svg>
      </button>

      <button
        v-if="isAuthenticated && isProjectRoute"
        class="editor-activity-btn"
        :class="{ 'is-active': activePanel === 'git' }"
        type="button"
        :title="t('editor.gitWork')"
        :aria-label="t('editor.gitWork')"
    ...
    <div v-if="isContentVisible" class="editor-sidebar-content">
      ...
      <section v-if="activePanel === 'collaborators' && canManageProjectSettings" class="sidebar-block">
      ...
      <section v-if="activePanel === 'git' && isAuthenticated && isProjectRoute" class="sidebar-block">
...
import {
  useEditorFilesystemContext,
  useEditorForgejoContext,
...
const filesystemContext = useEditorFilesystemContext();
const settingsContext = useEditorSettingsContext();
const forgejoContext = useEditorForgejoContext();
const context = {
  ...layoutContext,
  ...filesystemContext,
  ...settingsContext,
...
function emit(eventName, ...args) {
  const handler = sidebarEventHandlers[eventName];
...
const availablePanels = computed(() => {
  ...
  if (props.canManageProjectSettings) {
    panels.push("collaborators");
  }

  if (props.isAuthenticated && props.isProjectRoute) {
    panels.push("git");
  }
...
watch(
  availablePanels,
  (panels) => {
    if (activePanel.value && !panels.includes(activePanel.value)) {
      activePanel.value = panels[0] || "explorer";
    }
  },
  { immediate: true },
);
...
watch(
  isContentVisible,
  (value) => {
    emit("sidebar-content-visibility-change", value);
  },
  { immediate: true },
);
